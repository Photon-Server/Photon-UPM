namespace Fusion.Addons.InterestManagement
{
	...
	public class PlayerInterestManager : NetworkBehaviour
	{
		...
		private static bool IsInterestManagementActive(NetworkRunner runner)
		{
			...
			return runner.IsServer == true && runner.GameMode != GameMode.Shared && (runner.Config.Simulation.ReplicationFeatures & NetworkProjectConfig.ReplicationFeatures.InterestManagement) == NetworkProjectConfig.ReplicationFeatures.InterestManagement;
		}
		...
		private void DrawGizmos()
		{
			if (Application.isPlaying == true)
			{
				...
				if (IsInterestManagementActive(runner) == true)
				{
					...
					if (interestCells != null)
					{
						...
						{
							InterestUtility.DrawCells(runner.AreaOfInterest, interestCells);
						}
			...
			{
				if (_interestView != null && _interestView.TryGetAreaOfInterest(out Simulation.AreaOfInterestState areaOfInterest) == true)
				{
					...
					if (InterestUtility.DrawInterestCells == true)
					{
						InterestUtility.DrawCells(areaOfInterest, _gizmoCells);
					}

					if (InterestUtility.DrawPlayerInterest == true)
					{
						((IInterestProvider)_interestView).DrawGizmosForPlayer(_interestView);
					}
