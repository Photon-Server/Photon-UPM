...
namespace Fusion.Addons.DataSyncHelpers
{
    ...
    public struct RingBuffer
    {
        ...
        public Change DataUpdate(IRingBufferDataSource dataSource, out byte[] newBytes, ref byte[] completeCache, int cacheIncreaseStep = DEFAULT_CACHE_INCREASE_STEP)
        {
            ...
            if (positionInfo.totalData < newBytes.Length)
            {
                Debug.LogError($"[Error] Total data is small than its new part ({positionInfo.totalData} < {newBytes.Length}). PositionInfo: totalData={positionInfo.totalData} firstIndex={positionInfo.firstIndex} nextIndex={positionInfo.nextIndex}");
            }
