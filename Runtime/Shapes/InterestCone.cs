	...
	public class InterestCone : InterestShape
	{
		...
		public override void GetCells(PlayerInterestView playerView, HashSet<int> cells, EInterestMode interestMode)
		{
			if (IsEnabled == false || Length <= 0.0f)
				return;
			if (playerView.TryGetAreaOfInterest(out Simulation.AreaOfInterestState areaOfInterest) == false)
				return;
			...
			if (position.Equals(_cachedPosition) == false || rotation.Equals(_cachedRotation) == false || Length != _cachedLength || StartSize.Equals(_cachedStartSize) == false || EndSize.Equals(_cachedEndSize) == false || MinSize.Equals(_cachedMinSize) == false || MaxSize.Equals(_cachedMaxSize) == false || cellSize != _cachedCellSize || Version != _cachedVersion)
			{
				_cachedCells.Clear();

				InterestConeUtility.GetCells(areaOfInterest, position, rotation, Length, StartSize, EndSize, MinSize, MaxSize, _cachedCells);
		...
		public override void DrawGizmo(PlayerInterestView playerView)
		{
			...
			Transform.GetPositionAndRotation(out Vector3 position, out Quaternion rotation);
			InterestConeUtility.DrawGizmo(areaOfInterest, position, rotation, Length, StartSize, EndSize, MinSize, MaxSize, GizmoColor);
		}
