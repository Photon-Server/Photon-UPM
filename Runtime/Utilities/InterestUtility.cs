namespace Fusion.Addons.InterestManagement
{
	...
	public static class InterestUtility
	{
		...
		/// <summary>
		/// Returns Area of Interest from an interest provider or global project config.
		/// </summary>
		public static bool TryGetAreaOfInterest(this InterestProvider interestProvider, out Simulation.AreaOfInterestState areaOfInterest)
		{
			if (ReferenceEquals(interestProvider, null) == false)
			{
				NetworkRunner runner = interestProvider.Runner;
				if (ReferenceEquals(runner, null) == false)
				{
					areaOfInterest = runner.AreaOfInterest;
					return true;
				}
			}

			NetworkProjectConfig globalConfig = NetworkProjectConfig.Global;
			if (ReferenceEquals(globalConfig, null) == false)
			{
				areaOfInterest = new Simulation.AreaOfInterestState(globalConfig.Simulation.InterestManagementConfig);
				return true;
			}
		...
		/// </summary>
		public static void DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells)
		{
			DrawCells(areaOfInterest, cells, InterestCellGizmoColor, InterestCellGizmoSize);
		}

		/// <summary>
		/// Draws interest cells gizmos.
		/// </summary>
		public static void DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells, Color color)
		{
			DrawCells(areaOfInterest, cells, color, InterestCellGizmoSize);
		}

		/// <summary>
		/// Draws interest cells gizmos.
		/// </summary>
		public static void DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells, Color color, float size)
		{
			if (size <= 0.0f)
				return;
			if (cells.Count <= 0)
			...
			var cellSize = areaOfInterest.GetCellSize();
		...
		[Obsolete("Use DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells) instead.")]
		public static void DrawCells(HashSet<int> cells)
		{
			if (TryGetAreaOfInterest(null, out Simulation.AreaOfInterestState areaOfInterest) == false)
				return;

			DrawCells(areaOfInterest, cells, InterestCellGizmoColor, InterestCellGizmoSize);
		}

		[Obsolete("Use DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells, Color color) instead.")]
		...
		[Obsolete("Use DrawCells(Simulation.AreaOfInterestState areaOfInterest, HashSet<int> cells, Color color, float size) instead.")]
		public static void DrawCells(HashSet<int> cells, Color color, float size)
		{
			if (TryGetAreaOfInterest(null, out Simulation.AreaOfInterestState areaOfInterest) == false)
				return;

			DrawCells(areaOfInterest, cells, color, size);
		}
