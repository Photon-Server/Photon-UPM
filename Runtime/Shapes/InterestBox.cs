namespace Fusion.Addons.InterestManagement
{
	...
	public class InterestBox : InterestShape
	{
		...
		public override void GetCells(PlayerInterestView playerView, HashSet<int> cells, EInterestMode interestMode)
		{
			...
			if (position.Equals(_cachedPosition) == false || rotation.Equals(_cachedRotation) == false || IsAxisAligned != _cachedIsAxisAligned || Size.Equals(_cachedSize) == false || cellSize != _cachedCellSize || Version != _cachedVersion)
			{
				_cachedCells.Clear();

				InterestBoxUtility.GetCells(areaOfInterest, position, rotation, Size, IsAxisAligned, _cachedCells);
		...
		public override void DrawGizmo(PlayerInterestView playerView)
		{
			...
			Transform.GetPositionAndRotation(out Vector3 position, out Quaternion rotation);
			InterestBoxUtility.DrawGizmo(areaOfInterest, position, rotation, Size, IsAxisAligned, GizmoColor);
		}
