namespace Fusion.Addons.InterestManagement
{
	...
	public static class InterestSphereUtility
	{
		...
		/// </summary>
		public static void GetCells(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius, HashSet<int> cells)
		{
		...
		/// </summary>
		public static void DrawGizmo(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius, Color color, bool drawIcon = true)
		{
			...
			{
				DrawSamplePositions(areaOfInterest, position, radius);
			}
		...
		/// </summary>
		public static void DrawSamplePositions(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius)
		{
			Color gizmoColor = Gizmos.color;
					...
					for (int y = minY; y <= maxY; ++y)
					{
						for (int x = minX; x <= maxX; ++x)
						{
							...
							if (Vector3.SqrMagnitude(cellPosition - position) > sqrDistance)
							{
								...
								Gizmos.DrawSphere(cellPosition, cellSize * 0.05f);
							}
							else
							{
		...
		[Obsolete("Use GetCells(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius, HashSet<int> cells) instead.")]
		public static void GetCells(Vector3 position, float radius, HashSet<int> cells)
		{
			...
			GetCells(areaOfInterest, position, radius, cells);
		}

		[Obsolete("Use DrawGizmo(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius, Color color, bool drawIcon = true) instead.")]
		public static void DrawGizmo(Vector3 position, float radius, Color color, bool drawIcon = true)
		{
			if (InterestUtility.TryGetAreaOfInterest(null, out Simulation.AreaOfInterestState areaOfInterest) == false)
				return;

			DrawGizmo(areaOfInterest, position, radius, color, drawIcon);
		}

		[Obsolete("Use DrawSamplePositions(Simulation.AreaOfInterestState areaOfInterest, Vector3 position, float radius) instead.")]
