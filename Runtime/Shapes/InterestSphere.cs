namespace Fusion.Addons.InterestManagement
{
	...
	public class InterestSphere : InterestShape
	{
		...
		public override void GetCells(PlayerInterestView playerView, HashSet<int> cells, EInterestMode interestMode)
		{
			...
			if (position.Equals(_cachedPosition) == false || rotation.Equals(_cachedRotation) == false || Radius != _cachedRadius || cellSize != _cachedCellSize || Version != _cachedVersion)
			{
				_cachedCells.Clear();

				InterestSphereUtility.GetCells(areaOfInterest, position, Radius, _cachedCells);
		...
		public override void DrawGizmo(PlayerInterestView playerView)
		{
			...
			Transform.GetPositionAndRotation(out Vector3 position, out Quaternion rotation);
			InterestSphereUtility.DrawGizmo(areaOfInterest, position, Radius, GizmoColor);
		}
